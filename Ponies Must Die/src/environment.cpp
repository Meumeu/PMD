...
{
	Ogre::Entity * block1Entity = _sceneManager->createEntity("block1","block1.mesh");
	...
	_blocks.push_back(Block(block1Entity->clone("block3"), North, Ogre::Vector3(0,1,0)));
	_blocks.push_back(Block(block1Entity->clone("block4"), North, Ogre::Vector3(1,1,0)));
	
	_blocks.push_back(Block(block1Entity->clone("block5"), East,  Ogre::Vector3(2,0,0)));
	_blocks.push_back(Block(block1Entity->clone("block6"), West,  Ogre::Vector3(0,2,0)));
	_blocks.push_back(Block(block1Entity->clone("block7"), South, Ogre::Vector3(2,1,0)));
	...
	{
		sg->addEntity(block._entity, block._position);
		switch (block._orientation)
		{
			case North:
				...
				break;
			case East:
				sg->addEntity(block._entity, block._position, eastRotation);
				break;
			case South:
				sg->addEntity(block._entity, block._position, southRotation);
				break;
			case West:
				sg->addEntity(block._entity, block._position, westRotation);
		}
	}
	sg->build();
